...
const STAGES: { at: number; label: string }[] = [
  ...
  { at: 40, label: "Extracting text from your documents…" },
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      for (const slot of SLOTS) {
        for (const original of files[slot.key]) {
          const f = await compressImage(original);
          if (f.size > MAX_FILE_MB * 1024 * 1024) {
            throw new UserError(`"${f.name}" is larger than ${MAX_FILE_MB} MB. Please compress it or split it up.`);
          }
          totalBytes += f.size;
          form.append(slot.key, f, f.name);
        }
      }
      ...
      const { status, body } = await new Promise<{ status: number; body: any }>((resolve, reject) => {
        ...
        xhr.upload.onprogress = (e) => {
          if (e.lengthComputable) setPct(2 + Math.round((e.loaded / e.total) * 38));
        };
        xhr.upload.onload = () => {
          setPct(40);
          stopTimer();
    ...
    <main className="shell">
      ...
      {result && (
        <div className="report" ref={reportRef} tabIndex={-1} aria-label="Audit report">
          ...
          {result.docNotes.length > 0 && (
            <div className="doc-notes">
              {result.docNotes.map((n, i) => (
                <p key={i}>ⓘ {n}</p>
              ))}
            </div>
          )}
